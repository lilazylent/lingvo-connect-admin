"""Synchronize authoritative Lingvo Connect 2026 written-translation tariffs on deployed databases.

Revision ID: 0023_sync_tariffs_2026
Revises: 0022_non_billable_works

This migration intentionally creates dated authoritative seed rows instead of editing
user/manual tariffs. Existing legacy seed rows are deactivated after the 2026 rows
are available. A manual row with the exact same dated business key is preserved.
"""
...
revision = "0023_sync_tariffs_2026"
down_revision = "0022_non_billable_works"
branch_labels = None
depends_on = None

SOURCE_NOTE = "Тарифы Лингво Коннект 2026 — authoritative sync 2026-09-20"
LEGACY_NOTES = {
    "Тарифы Лингво Коннект 2026",
    "Тарифы Лингво Коннект 2026 — подтверждено по таблице клиента",
}
...
def upgrade():
    ...
    for source, target, direction, amount in _desired_rows():
        ...
        exact_key = sa.and_(
            ...
            tariffs.c.active_from == EFFECTIVE_FROM,
        )
        existing = bind.execute(
            sa.select(tariffs.c.id, tariffs.c.notes).where(exact_key)
        ).first()
        values = dict(
            amount=amount,
            min_quantity=1,
        ...
        if existing:
            # Preserve a user's explicitly dated manual tariff. Only our own seed is synced.
            if existing.notes == SOURCE_NOTE:
                bind.execute(sa.update(tariffs).where(tariffs.c.id == existing.id).values(**values))
            continue
        bind.execute(tariffs.insert().values(
            ...
            unit="CONDITIONAL_PAGE", active_from=EFFECTIVE_FROM, created_at=now, **values,
        ))

    # Retire only known legacy seed rows. User/manual tariffs are never modified here.
    legacy_rows = bind.execute(sa.select(
        tariffs.c.id, tariffs.c.source_language, tariffs.c.target_language, tariffs.c.direction, tariffs.c.notes
    ).where(
        tariffs.c.service_code == "written_translation",
        tariffs.c.unit == "CONDITIONAL_PAGE",
        tariffs.c.active.is_(True),
        tariffs.c.active_from.is_(None),
    )).all()
    stale_seed_ids = [
        row.id for row in legacy_rows
        if row.notes in LEGACY_NOTES and (row.source_language, row.target_language, row.direction) in desired_keys
    ]
    if stale_seed_ids:
        bind.execute(sa.update(tariffs).where(tariffs.c.id.in_(stale_seed_ids)).values(active=False, updated_at=now))
    ...
    for code, name, low, high, percent in exact_rules:
        row = bind.execute(sa.select(rules.c.id).where(rules.c.code == code)).first()
        values = dict(
        ...
        if row:
            bind.execute(sa.update(rules).where(rules.c.id == row.id).values(**values))
        else:
...
def downgrade():
    ...
    now = datetime.now(timezone.utc)
    tariffs = sa.table(
        "tariffs", sa.column("id", sa.String), sa.column("notes", sa.Text),
        sa.column("active", sa.Boolean), sa.column("active_from", sa.Date),
        sa.column("updated_at", sa.DateTime(timezone=True)),
    )
    # Remove only rows created by this synchronization and restore known legacy seed rows.
    bind.execute(sa.delete(tariffs).where(tariffs.c.notes == SOURCE_NOTE))
    bind.execute(
        sa.update(tariffs).where(
            tariffs.c.notes.in_(list(LEGACY_NOTES)),
            tariffs.c.active_from.is_(None),
