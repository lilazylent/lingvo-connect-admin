...
export default function ApplicationDetailPage() {
  ...
  return <>
    <header className="detail-head application-detail-head"><div><Link href="/admin/applications" className="detail-back">← Все заявки</Link><span className="overline overline--accent">Входящее обращение · {sourceLabels[item.source]}</span><h1>Заявка {item.number}</h1><p>{serviceLabel(item.requested_service)} · {formatDate(item.submitted_at)}</p></div><div className="detail-head__controls application-detail-head__controls"><div className="application-status-control"><span>Текущий статус</span><Badge tone={status.tone}>{status.label}</Badge></div><Select label="Статус заявки" hint="Сохраняется сразу после выбора" value={item.status_code} disabled={saving} onChange={(event) => void changeStatus(event.target.value as ApplicationStatus)}>{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select><ApplicationLinks id={item.id}/></div></header>
